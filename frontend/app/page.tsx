...
type Project = {
  id: string; title: string; script: string; status: string; platform: string;
  narration_audio_uri: string | null; narration_duration_ms: number | null;
};
...
export default function ProjectPage({ params }: { params: { id: string } }) {
  ...
  async function uploadNarration(file: File) {
    ...
    await fetch(`${API_URL}/projects/${params.id}/narration`, { method: "POST", body: form });
  ...
  async function startRender() {
    ...
    const poll = setInterval(async () => {
      ...
      if (j.status === "done") {
        setVideoUrl(`${API_URL}/render/${job.id}/download`);
        clearInterval(poll);
      }
      if (j.status === "failed") clearInterval(poll);
    }, 4000);
  }
    ...
    <main className="max-w-5xl mx-auto px-6 py-10">
      <div className="flex items-center justify-between mb-6">
        ...
        {project.status === "draft" && (
          <div className="flex items-center gap-3">
            <label className="text-sm text-neutral-400 cursor-pointer hover:text-neutral-200">
              {uploadingNarration
                ? "Uploading..."
                : project.narration_audio_uri
                ? `Voiceover: ${((project.narration_duration_ms ?? 0) / 1000).toFixed(0)}s ✓ (replace)`
      ...
      {tab === "Preview" && (
        videoUrl ? (
          ...
          <p className="text-neutral-600 text-sm">
            No rendered video yet in this session. Go to the Render tab and click "Render
            video" — once it finishes, it'll play here automatically.
          </p>
        )
      )}
        ...
        <div>
          ...
          {videoUrl && (
            <div className="mt-4 space-y-3">
              <video controls className="w-full rounded-lg bg-black" src={videoUrl} />
              <a href={videoUrl} download className="text-sm text-accent underline">
                Download video
              </a>
            </div>
          )}
