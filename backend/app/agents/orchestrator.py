...
def _fetch_unique_asset(fetch_fn, used_uris: set, max_pages: int = 5):
    """
    Calls fetch_fn(page=N) for N=1..max_pages until it returns a result
    whose uri hasn't been used yet elsewhere in this project, or gives up
    and returns the last result if every page was already used (better to
    repeat a photo once than fail the shot). Returns None if fetch_fn
    raised on every attempt.
    """
    last_result = None
    for page in range(1, max_pages + 1):
        try:
            result = fetch_fn(page=page)
        except Exception:
            continue
        last_result = result
        if result["uri"] not in used_uris:
            return result
    return last_result
...
def run_pipeline(db: Session, project: models.Project) -> models.Project:
    ...
    # Tracks image URIs already used anywhere in this project so far, so
    # that when multiple shots (often within the same scene, since they
    # share topics/entities and therefore near-identical search queries)
    # resolve to the same top Pexels result, we page forward instead of
    # showing the same photo repeatedly. Seeded from any shots already
    # persisted in a prior/resumed attempt so resumes don't reset it.
    used_asset_uris = {
        shot.asset.uri
        for scene in project.scenes
        for shot in scene.shots
        if shot.asset and shot.asset.uri
    }
    ...
    for i, raw_scene in enumerate(raw_scenes):
        ...
        for shot_data in raw_shots:
            ...
            # Anything else (chart/map/timeline/etc.) still falls back to
            # the placeholder renderer for now — logged, not fatal.
            asset_id = None
            orientation = "portrait" if project.platform in ("tiktok", "instagram") else "landscape"
            if shot_data["asset_type"] == "stock":
                query = shot_data["asset_spec"].get("query", "")
                result = _fetch_unique_asset(
                    lambda page: providers.search_stock_footage(query, orientation=orientation, page=page),
                    used_asset_uris,
                )
                if result is None:
                    logging.getLogger(__name__).warning(f"search_stock_footage failed for query: {query!r}")
            elif shot_data["asset_type"] == "ai_image":
                prompt = shot_data["asset_spec"].get("prompt", shot_data["asset_spec"].get("query", ""))
                style = shot_data["asset_spec"].get("style", "cinematic")
                result = _fetch_unique_asset(
                    lambda page: providers.generate_image(prompt, style=style, page=page),
                    used_asset_uris,
                )
                if result is None:
                    logging.getLogger(__name__).warning(f"generate_image failed for prompt: {prompt!r}")
            else:
                result = None

            if result is not None:
                used_asset_uris.add(result["uri"])
                asset = models.Asset(
                    type="image",
