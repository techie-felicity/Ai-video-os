...
def generate_image(prompt: str, style: str = "cinematic", page: int = 1) -> Dict[str, Any]:
    """
    True AI image generation (Flux/SDXL/Ideogram) isn't wired yet. Until it
    is, this falls back to a Pexels stock photo search using the prompt as
    the query, so "ai_image" shots get a real photo instead of a
    placeholder card. Swap the body of this function for a real generator
    later — callers already treat the return value the same as
    search_stock_footage's (a dict with provider/uri/metadata), so no
    caller changes will be needed when that happens.

    `page` is forwarded to the underlying search so the orchestrator can
    page forward and avoid handing out the same photo to multiple shots.
    """
    result = search_stock_footage(prompt, page=page)
    result["metadata"]["fallback_from"] = "generate_image"
    result["metadata"]["requested_style"] = style
    return result


def search_stock_footage(query: str, orientation: str = "landscape", page: int = 1) -> Dict[str, Any]:
    """
    ...
    resp = requests.get(
        ...
        params={"query": query, "per_page": 1, "orientation": orientation, "page": page},
        timeout=15,
    )
