...
async function main() {
  ...
  //
  // --crf=23 and --x264-preset=veryfast reduce the memory footprint of
  // Remotion's internal frame-stitching ffmpeg pass. The defaults
  // (crf=18, slower preset) use larger x264 lookahead buffers that scale
  // with render length — on longer videos this was enough to exceed
  // Railway's free-tier memory ceiling and get SIGKILLed ~90% through
  // stitching, even though frame capture itself completed fine.
  // Note: the mastering pass below uses -c:v copy (no re-encode), so this
  // IS the final video quality — crf=23 is still visually solid for
  // documentary-style footage, but bump toward 18-20 later once you've
  // confirmed memory headroom (e.g. after a plan upgrade).
  execSync(
    `npx remotion render src/index.ts Documentary "${rawFrames}" --props="${props}" --timeout=120000 --concurrency=1 --crf=23 --x264-preset=veryfast`,
