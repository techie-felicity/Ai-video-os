...
def _render_isolated(job_id: str):
    ...
    try:
        ...
        if result.returncode != 0:
            # ffmpeg prints repetitive "frame=... fps=... size=..." progress
            # lines that can flood out the actual error if we just take the
            # tail of stderr. Strip those before truncating so the real
            # Remotion/Node/ffmpeg error message is what we actually see.
            clean_lines = [
                line for line in result.stderr.splitlines()
                if not re.match(r"^\s*frame=\s*\d+\s+fps=", line)
            ]
            clean_stderr = "\n".join(clean_lines)
            raise RuntimeError(f"Render failed: {clean_stderr[-4000:]}")
